...
[operational workflow validation](../quality/OPERATIONAL-WORKFLOW-VALIDATION.md). The application
commit's repository-controlled CI, CodeQL, disposable Supabase/Auth/recovery and browser gates are
green; those reports distinguish that evidence from production-provider, credential and device
checks.
...
| ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Callback and administrator session boundary | Seventeen focused callback tests reject empty, duplicate, mixed and unsupported input and preserve the authenticated callback client; disabled, missing and unaccepted profiles fail closed; clean TOTP/admin acceptance passed                                  | Exact production redirects, email credentials and real committee identities                                                                           |
| Database roles, RLS and public privacy      | The disposable Supabase stack replayed migrations/seeds from zero twice and passed all 91 assertions after each reset for anonymous, non-admin, every application role, invited and disabled boundaries; schema lint passed                                      | Production-provider repetition when its credentials are supplied                                                                                      |
...
| Public-data projection                      | Unit/database/product tests select only explicit public fields, reject drafts, premature schedules, expiry boundaries and deleted rows, and deny anonymous table reads; all 370 applicable checks in the 385-check browser selection passed                      | Production repetition uses the approved content and Supabase credentials                                                                              |
...
| Supply chain and source hygiene             | Registry audit returned zero findings across 568 dependencies after PostCSS 8.5.17; dependency review, Gitleaks, CodeQL, link integrity, lint, typecheck, 132 tests and production build passed at the application commit                                        | Provider access controls, approved owners and ongoing update response                                                                                 |
| Logical recovery                            | The authoritative disposable-Supabase job restored realistic application data into a separately migrated database and verified relationships, privacy-safe audit snapshots, indexes and constraints                                                              | Provider backup/PITR, Auth/configuration and independent Storage-object restore with measured production RPO/RTO                                      |

The repository-controlled security gate is green at application commit `fd97cc6`, including
...
These passes do not authorise production launch; production credentials, approved owners/content and
committee sign-off remain required.
